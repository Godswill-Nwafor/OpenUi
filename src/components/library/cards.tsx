...
export function GlassCard({ title, description, icon, children, className = "" }: { title: string; description: string; icon?: React.ReactNode; children?: React.ReactNode; className?: string }) {
  return (
    <div className={`relative rounded-2xl p-6 overflow-hidden
      bg-background backdrop-blur-xl border border-border
      hover:bg-card/8 hover:border-border
...
export function ProfileCard({ name, role, avatar, bio, stats, github, twitter, website }: { name: string; role: string; avatar: string; bio?: string; stats?: { label: string; value: string }[]; github?: string; twitter?: string; website?: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card overflow-hidden w-72">
      ...
      <div className="px-5 pb-5">
        <div className="-mt-10 mb-3 relative w-fit">
          <img src={avatar} alt={name} className="w-20 h-20 rounded-2xl border-4 border-border object-cover" />
          <div className="absolute bottom-1 right-1 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-border" />
        </div>
        ...
        <div className="flex items-center gap-2">
          {github && <a href={github} className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-xl bg-muted text-foreground hover:text-foreground hover:bg-muted text-xs transition-colors"><Github size={13} /> GitHub</a>}
          {twitter && <a href={twitter} className="flex-1 flex items-center justify-center h-9 w-9 rounded-xl bg-muted text-foreground hover:text-foreground hover:bg-muted transition-colors"><Twitter size={14} /></a>}
          {website && <a href={website} className="flex-1 flex items-center justify-center h-9 w-9 rounded-xl bg-muted text-foreground hover:text-foreground hover:bg-muted transition-colors"><Globe size={14} /></a>}
        </div>
      </div>
    </div>
  );
...
export function PricingCard({ plan, price, period = "/mo", description, features, cta, popular, onSelect }: { plan: string; price: string; period?: string; description: string; features: string[]; cta: string; popular?: boolean; onSelect?: () => void }) {
  return (
    <div className={`relative rounded-2xl p-6 border transition-all duration-300 ${popular ? "border-indigo-500 bg-gradient-to-b from-indigo-950 to-gray-900 shadow-xl shadow-indigo-500/20" : "border-border bg-card hover:border-border"}`}>
      {popular && (
      ...
      <button onClick={onSelect} className={`w-full h-10 rounded-xl font-medium text-sm transition-all duration-150 active:scale-[0.98] ${popular ? "bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm shadow-indigo-500/25" : "bg-muted text-foreground hover:bg-muted"}`}>
        {cta}
      </button>
    </div>
  );
...
export function FeatureCard({ title, description, icon, gradient = "from-indigo-500 to-purple-600" }: { title: string; description: string; icon: React.ReactNode; gradient?: string }) {
  return (
    <div className="group relative rounded-2xl border border-border bg-card p-6 hover:border-indigo-500/40 transition-all duration-300 overflow-hidden">
      ...
      <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-foreground mb-5 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
        {icon}
      </div>
...
export function BlogCard({ title, excerpt, image, author, tag, readingTime, date, href = "#" }: { title: string; excerpt: string; image: string; author: { name: string; avatar: string }; tag: string; readingTime: number; date: string; href?: string }) {
  return (
    <a href={href} className="group block rounded-2xl border border-border bg-card hover:border-indigo-500/30 transition-all duration-300 overflow-hidden">
      ...
      <div className="p-5">
        ...
        <h3 className="font-semibold text-foreground line-clamp-2 mb-2 group-hover:text-indigo-400 transition-colors">{title}</h3>
        <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed mb-4">{excerpt}</p>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <img src={author.avatar} alt={author.name} className="w-6 h-6 rounded-full object-cover" />
            <span className="text-xs text-muted-foreground">{author.name}</span>
          </div>
          <ArrowRight size={14} className="text-muted-foreground group-hover:text-indigo-400 group-hover:translate-x-1 transition-all duration-200" />
        </div>
      </div>
    </a>
  );
...
export function NotificationCard({ avatar, name, action, target, time, read = false, type = "info" }: { avatar: string; name: string; action: string; target: string; time: string; read?: boolean; type?: "info" | "success" | "warning" }) {
  return (
    <div className={`flex items-start gap-3 p-4 rounded-xl transition-colors ${read ? "bg-transparent" : "bg-indigo-500/5 hover:bg-indigo-500/8"} border border-transparent ${!read ? "border-indigo-500/10" : "hover:border-border"}`}>
      <div className="relative shrink-0">
        <img src={avatar} alt={name} className="w-10 h-10 rounded-full object-cover" />
        <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full ${dotColor[type]} border-2 border-border`} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-foreground"><span className="font-semibold text-foreground">{name}</span>{" "}{action}{" "}<span className="font-medium text-indigo-400">{target}</span></p>
...
export function ProductCard({ name, price, originalPrice, image, rating, reviews, badge, onAddToCart }: { name: string; price: number; originalPrice?: number; image: string; rating: number; reviews: number; badge?: string; onAddToCart?: () => void }) {
  ...
  return (
    <div className="group rounded-2xl border border-border bg-card overflow-hidden w-60">
      <div className="relative h-52 overflow-hidden bg-muted">
        ...
        <button onClick={() => setLiked(v => !v)} className="absolute bottom-3 right-3 w-8 h-8 rounded-full bg-card/80 backdrop-blur-sm flex items-center justify-center transition-all hover:bg-muted">
          <Heart size={14} className={liked ? "text-red-500 fill-red-500" : "text-muted-foreground"} />
        </button>
      </div>
      <div className="p-4">
        <h3 className="text-sm font-semibold text-foreground line-clamp-1 mb-1">{name}</h3>
    ...
    <div className="group relative rounded-2xl overflow-hidden border border-border">
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-indigo-950/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-4 p-6">
          {bio && <p className="text-foreground text-sm text-center leading-relaxed">{bio}</p>}
          <div className="flex items-center gap-3">
            {socials?.github && <a href={socials.github} className="w-9 h-9 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors"><Github size={16} /></a>}
            {socials?.twitter && <a href={socials.twitter} className="w-9 h-9 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors"><Twitter size={16} /></a>}
            {socials?.linkedin && <a href={socials.linkedin} className="w-9 h-9 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors"><Linkedin size={16} /></a>}
          </div>
        </div>
      </div>
...
export function ProfileCardPreview() {
  return (
    <div className="flex items-center justify-center p-4 bg-background h-full">
      <ProfileCard
...
export function PricingCardPreview() {
  return (
    <div className="flex gap-3 p-4 bg-background h-full items-start">
...
export function StatCardPreview() {
  return (
    <div className="grid grid-cols-2 gap-3 p-4 bg-background h-full">
...
export function NotificationCardPreview() {
  return (
    <div className="flex flex-col gap-1.5 p-3 bg-background h-full">
...
export function TeamMemberCardPreview() {
  return (
    <div className="flex items-center justify-center gap-3 p-4 bg-background h-full">
      <div className="w-36">
