...
export function GradientHero() {
  return (
    <section className="relative flex items-center justify-center overflow-hidden bg-gray-50 py-20">
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <a href="https://github.com" className="flex items-center gap-2 h-12 px-7 rounded-2xl font-semibold text-gray-700 border border-gray-200 hover:bg-gray-100 transition-all duration-200 active:scale-95">
...
export function SplitHero() {
  ...
  return (
    <section className="flex items-center bg-gray-50 px-4 py-16">
      <div className="max-w-6xl mx-auto w-full grid lg:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="text-gray-500 text-lg leading-relaxed mb-8">Beautiful, accessible React components. Copy the code, own the design. No black boxes, no subscriptions.</p>
          <div className="flex items-center gap-3 p-3 pl-4 rounded-xl border border-gray-200 bg-white mb-8 max-w-sm">
            <Terminal size={15} className="text-gray-500 shrink-0" />
            <code className="text-sm text-gray-700 flex-1 font-mono">{installCmd}</code>
            <button onClick={copy} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-100 hover:bg-gray-100 text-gray-500 hover:text-gray-900 transition-all">
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-2xl shadow-gray-200">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 bg-white/50">
              ...
              <span className="text-xs text-gray-400 ml-2 font-mono">GlassButton.tsx</span>
            </div>
            <pre className="p-5 text-sm overflow-x-auto leading-relaxed">
              <code>
                ...
                <span className="text-gray-700">{"    <"}</span><span className="text-indigo-400">button</span>{"\n"}
                <span className="text-emerald-400">{"      className"}</span><span className="text-gray-700">{"="}</span><span className="text-amber-300">{"{"}{"`"}</span>{"\n"}
                <span className="text-amber-300">{"        px-6 py-2.5 rounded-xl"}</span>{"\n"}
                <span className="text-amber-300">{"        bg-gray-100 backdrop-blur-sm"}</span>{"\n"}
                <span className="text-amber-300">{"        text-gray-900 font-medium"}</span>{"\n"}
                <span className="text-amber-300">{"      "}{"`"}</span><span className="text-gray-700">{"}"}</span>{"\n"}
                <span className="text-gray-700">{"    >{children}"}</span>{"\n"}
                <span className="text-gray-700">{"    </"}</span><span className="text-indigo-400">button</span><span className="text-gray-700">{">"}</span>{"\n"}
...
export function MinimalHero({ eyebrow, title, highlight, subtitle, primaryCta, secondaryCta }: { eyebrow?: string; title: string; highlight?: string; subtitle: string; primaryCta?: { label: string; href: string }; secondaryCta?: { label: string; href: string } }) {
  ...
  return (
    <section className="py-32 px-4 bg-white dark:bg-gray-50">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="text-5xl sm:text-6xl font-bold text-gray-900 dark:text-gray-900 leading-tight mb-6">
          {highlight ? <>{parts[0]}<span className="text-indigo-600 dark:text-indigo-400">{highlight}</span>{parts[1]}</> : title}
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="text-lg text-gray-500 dark:text-gray-500 leading-relaxed mb-10">{subtitle}</motion.p>
        {(primaryCta || secondaryCta) && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-wrap items-center justify-center gap-4">
            ...
            {secondaryCta && <a href={secondaryCta.href} className="h-12 px-7 rounded-2xl border border-gray-200 dark:border-gray-200 text-gray-700 dark:text-gray-700 font-semibold hover:bg-gray-50 dark:hover:bg-gray-50 transition-colors flex items-center">{secondaryCta.label}</a>}
          </motion.div>
        )}
      </div>
    </section>
...
export function SaaSHero({ title, subtitle, primaryCta, secondaryCta, features, screenshotSrc }: { title: string; subtitle: string; primaryCta: string; secondaryCta?: string; features?: string[]; screenshotSrc?: string }) {
  return (
    <section className="bg-gray-50 pt-20 pb-0 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 text-center">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center gap-4 mb-8">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-gray-500">Loved by 10,000+ developers</p>
          </div>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="text-5xl sm:text-6xl font-bold text-gray-900 leading-tight mb-5"
          dangerouslySetInnerHTML={{ __html: title }} />
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="text-gray-500 text-lg max-w-xl mx-auto mb-8 leading-relaxed">{subtitle}</motion.p>
        {features && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 mb-10">
            {features.map(f => <span key={f} className="flex items-center gap-1.5 text-sm text-gray-500"><CheckCircle size={14} className="text-emerald-500" /> {f}</span>)}
          </motion.div>
        )}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-wrap items-center justify-center gap-4 mb-16">
          ...
          {secondaryCta && <a href="#" className="h-12 px-7 rounded-2xl font-semibold text-gray-700 border border-gray-200 hover:bg-gray-50 transition-all">{secondaryCta}</a>}
        </motion.div>
      </div>
    </section>
  );
  ...
  return (
    <section className="relative flex items-center justify-center overflow-hidden" style={{ minHeight: "100%" }}>
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto py-20">
        <h1 className="text-5xl sm:text-7xl font-black text-gray-900 leading-tight mb-6 tracking-tight">{title}</h1>
        <p className="text-lg sm:text-xl text-gray-900/70 leading-relaxed max-w-2xl mx-auto mb-10">{subtitle}</p>
...
export function VideoHeroPreview() {
  return (
    <div className="relative bg-gray-50" style={{ minHeight: "176px" }}>
      {/* Show static overlay since video won't load in preview */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/50 to-black/80" />
      <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.03)_1px,transparent_1px)] bg-[size:64px_64px]" />
      <div className="relative z-10 text-center px-4 py-12">
        <h2 className="text-4xl font-black text-gray-900 leading-tight mb-4">Create Without Limits</h2>
        <p className="text-gray-900/70 mb-6">The world&apos;s most beautiful UI component library.</p>
