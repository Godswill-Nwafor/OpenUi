    ...
    <section className="bg-gray-950 py-20 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-3 p-1 rounded-xl bg-gray-900 border border-white/10">
            <button onClick={() => setAnnual(false)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${!annual ? "bg-white text-gray-900" : "text-slate-400 hover:text-white"}`}>Monthly</button>
            <button onClick={() => setAnnual(true)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${annual ? "bg-white text-gray-900" : "text-slate-400 hover:text-white"}`}>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map(tier => (
            <div key={tier.name} className={`relative rounded-2xl p-7 border ${tier.highlight ? "bg-indigo-600 border-indigo-500 shadow-2xl shadow-indigo-500/20" : "bg-gray-900 border-white/10"}`}>
              ...
              <h3 className={`font-bold text-xl mb-1 ${tier.highlight ? "text-white" : "text-white"}`}>{tier.name}</h3>
              <p className={`text-sm mb-5 ${tier.highlight ? "text-indigo-200" : "text-slate-400"}`}>{tier.description}</p>
              <div className="mb-6">
                <span className={`text-4xl font-bold ${tier.highlight ? "text-white" : "text-white"}`}>${annual ? tier.price.annual : tier.price.monthly}</span>
                <span className={`text-sm ml-1 ${tier.highlight ? "text-indigo-200" : "text-slate-400"}`}>/mo</span>
              </div>
              <button className={`w-full h-10 rounded-xl font-semibold text-sm mb-7 transition-colors ${tier.highlight ? "bg-white text-indigo-600 hover:bg-gray-100" : "bg-white/10 text-white hover:bg-white/15"}`}>{tier.cta}</button>
              <ul className="space-y-3">
                {tier.features.map(f => (
                  <li key={f} className={`flex items-start gap-2.5 text-sm ${tier.highlight ? "text-indigo-100" : "text-slate-300"}`}>
                    <Check size={15} className={`shrink-0 mt-0.5 ${tier.highlight ? "text-white" : "text-emerald-400"}`} />{f}
                  </li>
                ))}
              </ul>
            </div>
...
function Cell({ val }: { val: boolean | string }) {
  if (typeof val === "boolean") return val ? <Check size={16} className="text-emerald-400 mx-auto" /> : <X size={16} className="text-slate-600 mx-auto" />;
    ...
    <section className="bg-gray-950 py-20 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400">Everything you get at each tier.</p>
        </div>
        <div className="rounded-2xl border border-white/10 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-900/80">
                <th className="px-5 py-4 text-left text-slate-400 font-medium w-2/5">Feature</th>
                {["Free", "Pro", "Enterprise"].map(p => <th key={p} className="px-5 py-4 text-center text-white font-semibold">{p}</th>)}
              </tr>
            </thead>
            <tbody>
              {features.map((row, i) => (
                <tr key={row.name} className={`border-t border-white/5 ${i % 2 === 0 ? "bg-gray-950" : "bg-gray-900/40"}`}>
                  <td className="px-5 py-3.5 text-slate-400">{row.name}</td>
  ...
  return (
    <section className="bg-gray-950 py-20 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold text-white mb-3">Usage-based pricing</h2>
          <p className="text-slate-400">Pay for what you use. Upgrade as you grow.</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-gray-900 p-8">
          <label className="block text-sm text-slate-400 font-medium mb-3">Monthly Active Users</label>
          ...
          <p className="text-slate-500 text-sm mb-8">For <span className="text-white font-medium">{current.range} MAU</span>. No hidden fees.</p>
...
export function UpgradeBanner({ trialDaysLeft = 7, ctaHref = "#" }: { trialDaysLeft?: number; ctaHref?: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-px">
      <div className="relative rounded-2xl bg-gray-950/50 backdrop-blur-sm px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <Zap size={20} className="text-amber-400 shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-white text-sm">Your free trial ends in {trialDaysLeft} days</p>
          <p className="text-slate-400 text-xs mt-0.5">Upgrade now to keep access to all Pro features and never lose your work.</p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button onClick={() => setDismissed(true)} className="text-slate-400 hover:text-white transition-colors text-sm">Dismiss</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="bg-gray-950 py-20 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold text-white mb-3">Build your plan</h2>
          <p className="text-slate-400">Start with Pro and add what you need.</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-gray-900 p-6 mb-4">
          <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
            <div><p className="font-semibold text-white">Pro Plan</p><p className="text-sm text-slate-400">All core features included</p></div>
            <span className="text-xl font-bold text-white">${basePrice}/mo</span>
          </div>
          <p className="text-sm font-medium text-slate-400 mb-3">Add-ons</p>
          <div className="space-y-3">
            {addons.map(addon => (
              <div key={addon.id} onClick={() => toggle(addon.id)}
                className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${selected.has(addon.id) ? "border-indigo-500/50 bg-indigo-500/10" : "border-white/10 hover:border-white/20 hover:bg-white/5"}`}>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${selected.has(addon.id) ? "bg-indigo-600/30 text-indigo-400" : "bg-gray-800 text-slate-400"}`}>{addon.icon}</div>
                <div className="flex-1 min-w-0">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all mt-1 ml-auto ${selected.has(addon.id) ? "border-indigo-500 bg-indigo-500" : "border-white/20"}`}>
                    {selected.has(addon.id) && <Check size={11} className="text-white" />}
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-2xl border border-white/10 bg-gray-900 p-6">
          <div className="flex items-center justify-between mb-4">
            <span className="text-slate-400">Total per month</span>
