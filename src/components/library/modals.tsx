  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby={title ? "modal-title" : undefined}
        className={`relative z-10 w-full ${sizes[size]} rounded-2xl border border-border bg-card shadow-2xl shadow-gray-200`}>
        {title && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <h2 id="modal-title" className="font-semibold text-foreground">{title}</h2>
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"><X size={16} /></button>
          </div>
        )}
...
export function Lightbox({ images, initialIndex = 0, open, onClose }: { images: { src: string; alt: string }[]; initialIndex?: number; open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-md">
      <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors z-10"><X size={18} /></button>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 text-foreground/60 text-sm">{index + 1} / {images.length}</div>
      {images.length > 1 && <button onClick={() => setIndex(i => (i - 1 + images.length) % images.length)} className="absolute left-4 w-10 h-10 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors"><ChevronLeft size={20} /></button>}
      <img src={images[index].src} alt={images[index].alt} className="max-w-full max-h-screen object-contain p-12 select-none" />
      {images.length > 1 && <button onClick={() => setIndex(i => (i + 1) % images.length)} className="absolute right-4 w-10 h-10 rounded-full bg-muted flex items-center justify-center text-foreground hover:bg-muted transition-colors"><ChevronRight size={20} /></button>}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          <kbd className="px-1.5 py-0.5 rounded text-xs text-muted-foreground border border-border font-mono">ESC</kbd>
        </div>
        <div className="max-h-80 overflow-y-auto py-2">
          {filtered.length === 0 ? (
            ...
            <button key={item.id} onClick={() => { item.action(); onClose(); }} onMouseEnter={() => setSelected(i)}
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${i === selected ? "bg-indigo-600/20 text-white" : "text-foreground hover:bg-muted/50"}`}>
              <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm ${i === selected ? "bg-indigo-600/30 text-indigo-400" : "bg-muted text-muted-foreground"}`}>{item.icon}</span>
              <span className="flex-1 text-sm">{item.label}</span>
              {item.category && <span className="text-xs text-muted-foreground">{item.category}</span>}
              {i === selected && <ArrowRight size={13} className="text-muted-foreground" />}
            </button>
          ))}
        </div>
...
export function Drawer({ open, onClose, title, children, side = "right", width = "w-80" }: { open: boolean; onClose: () => void; title?: string; children: React.ReactNode; side?: "left" | "right"; width?: string }) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex ${side === "right" ? "justify-end" : "justify-start"} ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
      <div className={`absolute inset-0 bg-background/60 backdrop-blur-sm transition-opacity duration-300 ${open ? "opacity-100" : "opacity-0"}`} onClick={onClose} />
      <div className={`relative z-10 ${width} h-full bg-card border-l border-border shadow-2xl flex flex-col transition-transform duration-300 ease-out ${open ? "translate-x-0" : side === "right" ? "translate-x-full" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          {title && <h2 className="font-semibold text-foreground">{title}</h2>}
          <button onClick={onClose} className="ml-auto w-8 h-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"><X size={16} /></button>
        </div>
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      {showCloseButton && (
        <div className="flex justify-end p-4">
          <button onClick={onClose} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors text-sm"><X size={15} /> Close</button>
        </div>
      )}
...
export function BaseModalPreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-6 bg-background">
      ...
      <p className="text-xs text-muted-foreground">Click to open dialog</p>
      <Modal open={open} onClose={() => setOpen(false)} title="Welcome to OpenUI Hub" size="md">
        <p className="text-muted-foreground text-sm leading-relaxed mb-6">OpenUI Hub is an open-source collection of beautiful, production-ready React components. Copy, customize, and ship in minutes.</p>
        <div className="flex gap-3 justify-end">
          <button onClick={() => setOpen(false)} className="px-4 py-2 rounded-xl border border-border text-foreground text-sm hover:bg-muted/50">Cancel</button>
...
export function ImageModalPreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-4 bg-background">
      ...
      <p className="text-xs text-muted-foreground">Click an image to view lightbox</p>
...
export function CommandPalettePreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-6 bg-background">
      <button onClick={() => setOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-border bg-card text-muted-foreground text-sm">
        <span>Search commands...</span>
        <kbd className="ml-4 flex items-center gap-1 text-xs border border-border rounded px-1.5 py-0.5">⌘K</kbd>
      </button>
...
export function DrawerModalPreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-6 bg-background">
      ...
      <p className="text-xs text-muted-foreground">Slides in from right</p>
      <Drawer open={open} onClose={() => setOpen(false)} title="Cart (3 items)" side="right">
        <div className="space-y-4">
          {["Component Pack", "Pro License", "Icon Set"].map(item => (
            <div key={item} className="flex items-center justify-between py-3 border-b border-border">
              <span className="text-sm text-foreground">{item}</span>
              <span className="text-sm font-medium text-foreground">$29</span>
            </div>
          ))}
        </div>
      </Drawer>
...
export function FullscreenModalPreview() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 p-6 bg-background">
      ...
      <p className="text-xs text-muted-foreground">Full-screen takeover</p>
      <FullscreenModal open={open} onClose={() => setOpen(false)}>
        <div className="flex items-center justify-center min-h-full p-8">
          <div className="text-center max-w-md">
            <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center mx-auto mb-6"><Layers size={28} className="text-foreground" /></div>
            <h1 className="text-3xl font-bold text-foreground mb-3">Welcome to OpenUI Hub</h1>
            <p className="text-muted-foreground leading-relaxed mb-8">Let&apos;s get you set up with beautiful components in minutes.</p>
